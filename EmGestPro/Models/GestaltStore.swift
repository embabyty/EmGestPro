...
final class GestaltStore: ObservableObject {
    ...
    /// before EmGestPro first modified the key.
    ...
    /// EmGestPro first touched it (removing keys that didn't exist before),
    ...
    /// the value saved before EmGestPro first wrote it. Siri / Apple
    ...
    func unspoofDevice() async throws -> ApplyResult {
        ...
        for key in Self.spoofKeys {
            ...
            if defaults.object(forKey: presenceKey) == nil {
                // Never snapshotted, so EmGestPro never wrote it — but the

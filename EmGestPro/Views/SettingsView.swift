...
struct SettingsView: View {
    ...
    private var patreonSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionHeader("EmGestPro Ultra")
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top, spacing: 12) {
                    Image(systemName: "person.crop.circle.badge.checkmark")
                        .foregroundStyle(Theme.accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(patreon.isUnlocked ? "Membership active" : "No active membership")
                            .font(.subheadline.weight(.medium))
                        if let tier = patreon.subscriberTier {
                            Text(tier)
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                        if let name = patreon.subscriberName, !name.isEmpty {
                            Text(name)
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                ...
                if patreon.isUnlocked {
                    Button("Sign out", role: .destructive) {
                        patreon.signOut()
                    }
                    .font(.subheadline.weight(.semibold))
                } else {
                    Text("Sign in with your Patreon account to unlock the app.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
